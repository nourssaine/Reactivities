...
export const useComments = (activityId?: string) =>{
    ...
    const commentStore = useLocalObservable (() =>({
        ...
        crateHubConnection(activityId: string){
            ...
            .build();
            this.hubConnection.start().catch(error => console.log('Error established:',error));
    ...
    useEffect(() => {
        if (activityId && !created.current) {
            commentStore.crateHubConnection(activityId);
